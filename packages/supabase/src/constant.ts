...
export const FETCH_ALL_ONE_SQL = `SELECT 'accidents' AS table_name, row_to_json(t) AS data FROM (SELECT * FROM public.accidents LIMIT 1) t
UNION ALL
...
UNION ALL
SELECT 'pay_sequence', row_to_json(t) FROM (SELECT * FROM public.pay_sequence LIMIT 1) t
UNION ALL
