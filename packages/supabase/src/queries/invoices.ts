...
export async function getInvoicesByLocationId({
  supabase,
  locationId,
  params,
}: {
...
export async function getPaidInvoicesAmountsByLocationIdByMonthsForReimbursements({
  supabase,
  locationId,
  filters,
}: {
  ...
  if (currentMonthError)
    console.error(
      "getPaidInvoicesAmountsByLocationIdByMonthsForReimbursements Error",
      currentMonthError,
    );
...
export async function getInvoicesByLocationIdForDashboard({
  supabase,
  locationId,
  filters,
}: {
  ...
  if (error) {
    console.error("getInvoiceDataByLocationIdForDashboard Error", error);
    return { data: null, error };
  }
  const groupedByMonthObj: Record<string, typeof data> = {};
