...
export const createAttendanceHourlyWorkbook = async ({
  selectedRows,
  companyName,
  companyAddress,
}: {
  ...
  const updatedData = selectedRows.map((employee) => {
    const totalHours = Object.values(employee).reduce((sum: number, value) => {
      ...
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            onClick={() => generateAttendanceHourlyExcel(selectedRows)}
            onSelect={() => generateAttendanceHourlyExcel(selectedRows)}
          >
            Create
          </AlertDialogAction>
        </AlertDialogFooter>
