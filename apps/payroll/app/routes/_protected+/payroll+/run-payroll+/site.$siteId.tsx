...
import type { PayrollDatabaseInsert, TypedSupabaseClient } from "@canny_ecosystem/supabase/types";
...
async function populateTemplateComponentData(supabase: TypedSupabaseClient, payrollEntry: PayrollEmployeeData, templateComponents: PaymentTemplateComponentType[]) {
  await Promise.all((templateComponents ?? []).map(async (templateComponent) => {
...
async function processPayrollEntries(supabase: TypedSupabaseClient, siteId: string, companyId: string, workingDays: number) {
  const { data: employeeData } = await getEmployeeIdsByProjectSiteId({ supabase, projectSiteId: siteId });

  return Promise.all((employeeData ?? []).map(async (e) => {
    ...
    const [employeeInfo, assignmentData] = await Promise.all([
      ...
      getEmployeeProjectAssignmentByEmployeeId({ supabase, employeeId: e.employee_id })
    ]);
    ...
    // Template processing
    const employeeTemplate = await getPaymentTemplateByEmployeeId({ supabase, employeeId: e.employee_id });
    ...
    if (employeeTemplate.data) {
      ...
      if (templateComponents) await populateTemplateComponentData(supabase, payrollEntry, templateComponents);
    } else {
      const siteTemplate = await getPaymentTemplateBySiteId({ supabase, site_id: siteId });
      ...
      if (siteTemplate.data) {
        ...
        const defaultTemplate = await getDefaultTemplateIdByCompanyId({ supabase, companyId });
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const payrollDataPromise = Promise.all([
    ...
    const workingDays = getWorkingDaysInCurrentMonth(sitePaySequenceData.data?.working_days);
    const payrollEntries = await processPayrollEntries(supabase, siteId, companyData.companyId, workingDays);

    const PAYROLL_ID = payrollData.data?.id;
    if (PAYROLL_ID) {
      await Promise.all(payrollEntries.map(async (entry) => {
        await Promise.all(entry.templateComponents.map(async (component: any) => {
          if (component.is_pro_rata) {
            component.amount = newAmount(component.value ?? 0, entry.present_days, workingDays);
          } else {
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (relationshipData.data) {
    ...
    dataToBeSaved.commission = (relationshipTerms.data?.terms?.service_charge * (dataToBeSaved?.total_net_amount ?? 0)) / 100;
  }
  ...
  await Promise.all(payrollEntries.map(async (payrollEntry) => {
    const baseEntry = {
    ...
    await Promise.all(payrollEntry.templateComponents.map(async (templateComponent: any) => {
      const entry = {
        ...baseEntry,
        payment_template_components_id: templateComponent.paymentTemplateComponentId,
        amount: templateComponent.is_pro_rata
          ? newAmount(templateComponent.value ?? 0, payrollEntry.present_days, workingDays)
...
export default function PayrollId() {
  ...
  return (
    <Suspense fallback={<div className="w-full p-4 text-center">Loading payroll data...</div>}>
      <Await
      ...
      >
        {(payrollEntries) => <PayrollComponent data={payrollEntries as any} editable={true} />}
      </Await>
    </Suspense>
  );
}