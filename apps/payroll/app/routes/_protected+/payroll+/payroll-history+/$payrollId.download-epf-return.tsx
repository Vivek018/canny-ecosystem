import { json, useLoaderData, useNavigate, useNavigation } from "@remix-run/react";
            ...
            await Promise.all(
                (payrollEntriesData ?? []).map(async (payrollEntry) => {
                    ...
                    if (paymentTemplateComponentData?.target_type === "payment_field") {
                        ...
                        } else if (paymentFieldData?.consider_for_epf &&
...
export default function DownloadRoute() {
    ...
    useEffect(() => {
        if (navigation.state === "idle" && !hasDownloaded.current) {
            ...
            epfReturn.map((data) => {
                const epfWages = data.salaryDetails.basicPay + data.salaryDetails.dearnessAllowance;
