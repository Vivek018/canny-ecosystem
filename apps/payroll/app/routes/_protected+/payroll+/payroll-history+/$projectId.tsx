...
async function enrichSiteData(supabase: TypedSupabaseClient, siteData: SitesWithLocation[]) {
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const { data: siteData, error: siteError } = await getSitesWithEmployeeCountByProjectId({
    supabase,
    projectId
  });
  ...
  return (
    <section className='p-4'>
      <div className='w-full flex items-end justify-between'>
        <Suspense fallback={<div className="w-full py-20 text-center">Loading sites...</div>}>
          <Await resolve={sitePromise}>{(sites) => {
