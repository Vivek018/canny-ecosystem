...
async function getTemplateComponents(supabase: TypedSupabaseClient, employeeId: string, siteId: string, companyId: string) {
  ...
  const { data: defaultPaymentTemplateData } = await getDefaultTemplateIdByCompanyId({
    supabase,
    companyId
  });
...
async function populateTemplateComponentData(supabase:TypedSupabaseClient, payrollEntry: PayrollEmployeeData, templateComponents: PaymentTemplateComponentType[]) {
  const componentPromises = templateComponents.map(async (templateComponent) => {
...
async function preparePayrollData(supabase:TypedSupabaseClient, siteId: string, payrollId: string, companyId: string) {
  ...
  const payrollEntriesPromise = Promise.all(
    (employeeIds ?? []).map(async (e) => {
      ...
      // Get employee data
      const [employeeData, employeeProjectAssignment, templateComponents] = await Promise.all([
      ...
      if (employeeProjectAssignment.data) {
        payrollEntry.designation = employeeProjectAssignment.data.position ?? "";
      }

      if (templateComponents.data) {
        await populateTemplateComponentData(supabase, payrollEntry, templateComponents.data);
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const payrollEntriesPromise = preparePayrollData(supabase, siteId, payrollId, companyId);
...
export default function PayrollId() {
  ...
  return (
    <Suspense fallback={<div className="w-full py-20 text-center">Loading payroll data...</div>}>
      <Await resolve={payrollEntriesPromise}>
        {(payrollEntries) => <PayrollComponent data={payrollEntries as any} editable={false} />}
      </Await>
    </Suspense>
  );
}