...
import { Await, defer, json, useActionData, useLoaderData, useNavigate } from "@remix-run/react";
import { parseWithZod } from "@conform-to/zod";
import { ExitFormSchema, hasPermission, isGoodStatus, updateRole } from "@canny_ecosystem/utils";
...
export async function action({ request, params }: ActionFunctionArgs): Promise<Response> {
    ...
    try {
        ...
        if (isGoodStatus(status)) return json({ status: "success", message: "Exit updated", error: null });

        return json({ status: "error", message: "Exit update failed", error }, { status: 500 });
    } catch (error) {
        return json(
            {
                status: "error",
    ...
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <Await resolve={exitPromise}>
                {(resolvedData) => {
                    if (!resolvedData) return <ErrorBoundary message="Failed to load Exit" />;
                    return <UpdateExitWrapper data={resolvedData?.data} error={resolvedData?.error} />
                }}
            </Await>
        </Suspense>
    );
