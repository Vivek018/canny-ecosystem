...
export async function action({ request }: ActionFunctionArgs) {
    ...
    const { status, error } = await updatePaymentField({ supabase, data: submission.value });

    const returnTo = formData.get("returnTo");
    if (isGoodStatus(status)) return safeRedirect(returnTo ?? "/payment-fields", { status: 303 });
