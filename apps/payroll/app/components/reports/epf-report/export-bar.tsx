...
import { epfReportColumnIdArray } from "./table/data-table-header";
...
export function ExportBar({
  ...
  const toBeExportedData = data.map((element) => {
    const exportedData: {
      [key: (typeof epfReportColumnIdArray)[number]]: string | number | boolean;
    } = {};

    for (const key of epfReportColumnIdArray) {
  ...
  const handleExport = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    link.setAttribute("download", `EPF Report - ${formatDateTime(Date.now())}`);
