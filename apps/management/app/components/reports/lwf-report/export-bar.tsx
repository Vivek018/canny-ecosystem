...
import { lwfReportColumnIdArray } from "./table/data-table-header";
...
export function ExportBar({
  ...
  const toBeExportedData = data.map((element) => {
    const exportedData: {
      [key: (typeof lwfReportColumnIdArray)[number]]: string | number | boolean;
    } = {};

    for (const key of lwfReportColumnIdArray) {
  ...
  const handleExport = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    link.setAttribute("download", `LWF Report - ${formatDateTime(Date.now())}`);
