...
import { ptReportColumnIdArray } from "./table/data-table-header";
...
export function ExportBar({
  ...
  //   const totalEligibleCount = data.length;
  const toBeExportedData = data.map((element) => {
    const exportedData: {
      [key: (typeof ptReportColumnIdArray)[number]]: string | number | boolean;
    } = {};

    for (const key of ptReportColumnIdArray) {
  ...
  const handleExport = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    link.setAttribute(
      "download",
      `Professional Tax Report - ${formatDateTime(Date.now())}`,
    );
