...
import { extractKeys, ImportPayrollDataSchema } from "@canny_ecosystem/utils";
  ...
  const handleFinalImport = async () => {
    if (validateImportData(importData.data)) {
      ...
      if (payrollType === "reimbursement") {
        ...
        const reimbursementForPayroll = extractKeys(updatedData, [
          "id",
          "employee_id",
          "amount",
        ]);
      ...
      if (payrollType === "exit") {
        const updatedData = importData.data!.map((item: any) => {
          ...
          const { employee_code, amount, ...rest } = item;
        ...
        const exitForPayroll = extractKeys(updatedData, [
          "id",
          "employee_id",
          "net_pay",
        ]);
        ...
        const totalNetAmount = exitForPayroll?.reduce(
          (sum, item) => sum + Number(item?.net_pay),
          0
        );
