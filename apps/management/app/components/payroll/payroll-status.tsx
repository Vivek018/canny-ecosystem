...
export function PayrollStatus({
  ...
  const handleClick = () => {
    submit(
      ...
      {
        method: "post",
        action: data.is_approved
          ? `/payroll/payroll-history/site/${data.id}/${data.payrollId}`
          : `/payroll/run-payroll/site/${data.id}`,
        navigate: true,
      },
    );
  };
  ...
  return (
    <Card className="w-full m-4 select-text cursor-auto dark:border-[1.5px] h-40 flex flex-col justify-between">
          ...
          <div className="text-md mx-3 text-bolder tracking-wide flex-col justify-center items-center text-center">
            ...
            <p
              className={cn(
                ...
                data.is_approved ? "bg-green" : "bg-muted",
              )}
            >
      ...
      <div
        className={cn(
          "my-2 mx-9 text-muted-foreground",
          data.is_approved ? "hidden" : "",
        )}
      >
