    ...
    <div className="w-full flex items-center justify-between gap-3 pb-4">
      <div className={cn("w-2/3", !showFilters && "hidden")}>
        <MultiSelectCombobox
          ...
          renderItem={(option) => (
            <div role="option" aria-selected={selectedDeptIds.includes(String(option.value))}>
              {option.label}
            </div>
          )}
        ...
        <span id="payment-field-description" className="sr-only">
          Select one or more payment fields. Shows individual payment fields names when 3 or fewer are selected.
        </span>
      </div>
