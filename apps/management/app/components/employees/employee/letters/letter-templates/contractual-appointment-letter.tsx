...
export function ContractualAppointmentLetter({
  data,
  employeeAddressData,
  companyData,
}: {
  ...
  const replacements = {
    ...
    employeeJoiningDate: new Date(
      data?.employees.work_details[0]?.start_date ?? "",
    ).toLocaleDateString("en-IN"),
    employeeLeavingDate: new Date(
      data?.employees.work_details[0]?.end_date ?? "",
    ).toLocaleDateString("en-IN"),
    employeePosition: data?.employees.work_details[0]?.position ?? "",
    ...
    projectName: data?.employees.work_details[0]?.sites?.projects?.name ?? "",
    siteName: data?.employees.work_details[0]?.sites?.name ?? "",
    basicSalary: "0",
  };
