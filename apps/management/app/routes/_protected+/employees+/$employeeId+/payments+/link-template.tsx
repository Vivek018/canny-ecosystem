...
import {
  ...
  hasPermission, replaceUnderscore, updateRole,
} from "@canny_ecosystem/utils";
import {
  getPaymentTemplateAssignmentByEmployeeId, getPaymentTemplatesByCompanyId,
...
export async function loader({ request, params }: LoaderFunctionArgs): Promise<Response> {
  ...
  if (!hasPermission(`${user?.role!}`, `${updateRole}:${attribute.employeeProjectAssignment}`))
    return safeRedirect(DEFAULT_ROUTE, { headers });

  try {
    const paymentTemplateAssignmentData = await getPaymentTemplateAssignmentByEmployeeId({
      supabase, employeeId
    });

    const paymentTemplatesData = await getPaymentTemplatesByCompanyId({ supabase, companyId });
...
export default function EmployeeLinkTemplateForm() {
  ...
  useEffect(() => {
    ...
    if (paymentTemplatesData) {
      const newPaymentTemplatesOptions = paymentTemplatesData?.map((paymentTemplate: any) => ({
    ...
    <section className="px-4 lg:px-10 xl:px-14 2xl:px-40 py-4">
      <FormProvider context={form.context}>
        <Form
          ...
          action={`/employees/${employeeId}/payments/${initialValues ? "update" : "create"}-link-template`}
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {initialValues ? "Update" : "Create"} payment template link for the employee
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Field
                className="w-full mb-6"
                inputProps={{ ...getInputProps(fields.name, { type: "text" }), placeholder: "Name" }}
              ...
              <div className="grid grid-cols-2 place-content-center justify-between gap-6 mb-6">
                <Field
                  ...
                  inputProps={{
                    ...
                    defaultValue: getValidDateForInput(fields.effective_from.initialValue as any),
                  }}
                ...
                <Field
                  ...
                  inputProps={{
                    ...
                    min: getValidDateForInput(fields.effective_from.value as any),
                    defaultValue: getValidDateForInput(fields.effective_to.initialValue as any),
                  }}
              ...
              <SearchableSelectField
                ...
                inputProps={{ ...getInputProps(fields.template_id, { type: "text" }) }}
