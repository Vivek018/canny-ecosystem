...
import type { ImportEmployeeGuardiansHeaderSchemaObject } from "@canny_ecosystem/utils";
...
import { getEmployeeGuardiansConflicts } from "@canny_ecosystem/supabase/mutations";
...
type FieldConfig = {
  key: keyof z.infer<typeof ImportEmployeeGuardiansHeaderSchemaObject>;
  required?: boolean;
};
...
export default function EmployeeGuardiansImportFieldMapping() {
  ...
  const { setImportData } = useImportStoreForEmployeeGuardians();
  ...
  const validateMapping = () => {
    try {
      const mappingResult = ImportEmployeeGuardiansHeaderSchema.safeParse(
        Object.fromEntries(
      ...
      Papa.parse(file, {
        ...
        complete: async (results) => {
          ...
          if (validateImportData(finalData)) {
            setImportData({
              data: finalData as ImportEmployeeGuardiansDataType[],
            });

            const { conflictingIndices, error } =
              await getEmployeeGuardiansConflicts({
                supabase,
                importedData: finalData as EmployeeBankDetailsDatabaseInsert[],
              });
