...
import { LeavesImportData } from "@/components/leaves/import-export/leaves-import-data";

type FieldConfig = {
  key: keyof z.infer<typeof ImportLeavesHeaderSchema>;
  required?: boolean;
};
...
export default function LeavesFieldMapping() {
  const { env, companyId } = useLoaderData<typeof loader>();

  const { setImportData } = useImportStoreForLeaves();
  ...
  useEffect(() => {
    if (file) {
      Papa.parse(file, {
        ...
        error: (error) => {
          console.error("Leaves Header parsing error:", error);
  ...
  const validateMapping = () => {
    try {
      const mappingResult = ImportLeavesHeaderSchema.safeParse(
        Object.fromEntries(
      ...
      console.error("Leaves Validation error:", error);
      ...
      Papa.parse(file, {
        ...
        complete: async (results) => {
          ...
          if (validateImportData(finalData)) {
            setImportData({
              data: finalData as ImportLeavesDataType[],
            });
        ...
        error: (error) => {
          console.error("Leaves Data parsing error:", error);
          setErrors((prev) => ({
            ...prev,
      ...
      {loadNext ? (
        ...
        <Card className="m-4 px-40">
          <CardHeader>
            ...
            <CardDescription>
              Map your fields with the Leaves fields
            </CardDescription>
          </CardHeader>
          <CardContent>
