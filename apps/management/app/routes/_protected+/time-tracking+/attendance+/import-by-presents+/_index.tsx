...
import type { ImportEmployeeAttendanceByPresentsHeaderSchemaObject } from "@canny_ecosystem/utils";
...
import {
  ...
  pipe,
  replaceDash,
  ImportEmployeeAttendanceByPresentsHeaderSchema,
  ImportEmployeeAttendanceByPresentsDataSchema,
  defaultMonth,
  defaultYear,
...
import { EmployeeAttendanceByPresentsImportData } from "@/components/employees/import-export/employee-attendance-by-presents-import-data";
type FieldConfig = {
  key: keyof z.infer<
    typeof ImportEmployeeAttendanceByPresentsHeaderSchemaObject
  >;
  required?: boolean;
};
...
export default function AttendanceByPresentsImportFieldMapping() {
  const { env } = useLoaderData<typeof loader>();
  const { setImportData } = useImportStoreForEmployeeAttendanceByPresents();
  ...
  const validateMapping = () => {
    try {
      const mappingResult =
        ImportEmployeeAttendanceByPresentsHeaderSchema.safeParse(
          Object.fromEntries(
  ...
  const validateImportData = (data: any[]) => {
    try {
      const result = ImportEmployeeAttendanceByPresentsDataSchema.safeParse({
        data,
      });
      if (!result.success) {
      ...
      Papa.parse(file, {
        ...
        complete: async (results) => {
          ...
          if (validateImportData(finalData)) {
            setImportData({
              data: updatedData as ImportEmployeeAttendanceByPresentsDataType[],
            });
        ...
        error: (error) => {
          console.error("Attedance by Presents Data parsing error:", error);
          setErrors((prev) => ({
            ...prev,
  ...
  return (
    <section className="py-4 ">
      {loadNext ? (
        <EmployeeAttendanceByPresentsImportData env={env} />
      ) : (
