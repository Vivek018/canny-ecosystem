  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View
          style={[styles.tableHeader, { borderLeft: "0.5pt solid #000000" }]}
        >
        ...
        {data.employeeData.map((employee, index) => (
          <View
            key={employee.employeeData.employee_code}
            style={[styles.tableRow, { borderLeft: "0.5pt solid #000000" }]}
          >
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const { data: cannyData } = await getCompanyRegistrationDetailsByCompanyId({
    supabase,
...
export default function SalaryRegister() {
  const { salaryRegister: data } = useLoaderData<{
    salaryRegister: DataType;
  }>();
    ...
    <Dialog defaultOpen={true} onOpenChange={handleOpenChange}>
      ...
      >
        <PDFViewer width="100%" height="100%">
          <SalaryRegisterPDF data={data as DataType} />
        </PDFViewer>
      </DialogContent>
    </Dialog>
  );
